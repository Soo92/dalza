...
public class Boss extends Enemy{
	...
	//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@오버 라이딩
		...
		public void set_Range_Site_Width_Right_Point(int range_Site_Width_Right_Point){
			this.range_Site_Width_Right_Point = range_Site_Width_Right_Point + width;
		}
		...
		public void enemy_Move(){ //적의 경계 범위는 바닥 정보의 범위 값을 가져와서 계산한다.
			if(playerMove) {
				enemy_Point.y -= 6-5*(cnt-jumpping); //v = v0 - at
		         if (enemy_Point.y<=b-50) { 
		        	 playerMove = false;
		         }
	         }else {
	        	 enemy_Point.y += 6-5*(cnt-jumpping); //v = v0 - at
			...
			if(!find_Hero){
					...
					if((left_Bound_Site - 10) >= enemy_Point.x){
						move_Site = false;
						if(playerMove) {
							enemy_Point.y -= 6-5*(cnt-jumpping); //v = v0 - at  enemy_Point.y값을 enemy공격범위 값으로 변경할 것.
					         System.out.println(enemy_Point.y-b);
					         if (enemy_Point.y<=b-50) { 
					        	 playerMove = false;
					         }
				         }else {
